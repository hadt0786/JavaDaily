...
public class MergeSort {
	...
	public static void sort(int inputArray[], int start , int end ) {
		...
		merge(inputArray, start, mid,end);//end.merge sorted results into the input array
	...
	public static void merge(int inputArray[],int start, int mid, int end) {
		 ...
		 * So both the right and left side of the array considered to be sorted */
		
		if(leftSlot<=mid) {//considered the right side done being sorted 
			while(leftSlot<=mid) {
				tempArray[k] = inputArray[leftSlot];
				k = k+1;
			}
	...
	// copy over the temp array into the appropriate slots of input array
